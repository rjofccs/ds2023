...
unsigned int printInfo(void *priv, struct sk_buff *skb, const struct nf_hook_state *state)
{
   ...
   printk(KERN_INFO "task2Print    %pI4  --> %pI4 (%s)\n", &(iph->saddr), &(iph->daddr), protocol);
...
int registerFilter(void) {
   printk(KERN_INFO "task2Print Registering filters.\n");
...
void removeFilter(void) {
   printk(KERN_INFO "task2Print The filters are being removed.\n");
